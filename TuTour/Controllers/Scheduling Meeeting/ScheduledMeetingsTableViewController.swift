        ...
        currentUserRef.observe(.childAdded) { (snapshot) in
            ...
            meetingRequestRef.observeSingleEvent(of: .value, with: { (snapshot) in
                if let dictionary = snapshot.value as? [String: AnyObject]{
                    let scheduledMeeeting = ScheduledMeeting(dictionary: dictionary)
...
//
        ...
        print(scheduledMeetings[indexPath.row].location)
        
        meetingRequestVC.setUser(user: scheduledMeetingUsers[indexPath.row])
        meetingRequestVC.setMeetingRequest(meetingRequest: scheduledMeetings[indexPath.row])
